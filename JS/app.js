...
let cart = JSON.parse(localStorage.getItem('cart')) || [];
 const firebaseConfig = {
    apiKey: "AIzaSyDN2Mu8cd9g8DiSJFGMGbu7PPGt-T_mg3s",
    authDomain: "my-account-cc82d.firebaseapp.com",
    databaseURL: "https://my-account-cc82d-default-rtdb.firebaseio.com",
    projectId: "my-account-cc82d",
    storageBucket: "my-account-cc82d.firebasestorage.app",
    messagingSenderId: "240356780202",
    appId: "1:240356780202:web:29d6c8bf57636b384848d3"
  };
...
function filterByCategory(category) {
    ...
    if (productsList) {
        ...
        if (pageTitle) {
            pageTitle.textContent = category ? category : 'All Products';
        }
...
function updateQuantity(productId, change, size) {
    const sizeToCompare = size === 'undefined' ? undefined : size;
    const item = cart.find(item => item.id === productId && item.size === sizeToCompare);
    if (item) {
...
function removeFromCart(productId, size) {
    const sizeToCompare = size === 'undefined' ? undefined : size;
    cart = cart.filter(item => !(item.id === productId && item.size === sizeToCompare));
    localStorage.setItem('cart', JSON.stringify(cart));
    loadCartItems();
    updateCartCount();
...
function proceedToCheckout() {
    ...
    if (!currentUser.phone || !currentUser.address || !currentUser.city || !currentUser.pincode) {
        showNotification('Please complete your profile before proceeding to checkout.', 'error');
...
function placeOrder() {
    ...
    const currentUser = JSON.parse(localStorage.getItem('currentUser'));
    if (!currentUser.phone || !currentUser.address || !currentUser.city || !currentUser.pincode) {
        showNotification('Please complete your profile before placing an order.', 'error');
        window.location.href = 'profile.html';
        return;
    }
    ...
    const orderDetails = {
        ...
        items: cart.map(item => ({ ...item, size: item.size })),
        subtotal: subtotal,
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (window.location.pathname.includes('login.html')) {
        ...
        if (section === 'categories') {
            ...
            if (pageTitle) pageTitle.style.display = 'none'; // Hide product title
            ...
            const categoriesSection = document.querySelector('.categories-section');
            if (productsSection) productsSection.style.display = 'block';
            if (pageTitle) pageTitle.style.display = 'block'; // Show product title
            if (categoriesSection) categoriesSection.style.display = 'none';
        }
