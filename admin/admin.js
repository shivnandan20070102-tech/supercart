            ...
            tableBody.innerHTML = orders.reverse().map(order => { // .reverse() to show newest first
                ...
                return `
                    <tr>
                        ...
                        <td data-label="Phone">${order.customerInfo.phone}</td>
...
async function loadUsersData() {
    try {
        ...
        const orders = JSON.parse(localStorage.getItem('orders')) || []; // Orders can still come from local for now
        const tableBody = document.getElementById('usersTableBody');

        if (tableBody) {
            if (users.length === 0) {
                tableBody.innerHTML = '<tr><td colspan="8" style="text-align: center;">No users found on the server!</td></tr>';
            } else {
                tableBody.innerHTML = users.map(user => {
                    const userOrders = orders.filter(order => order.userId === user.id);
                    const profilePic = user.profileImage || 'https://via.placeholder.com/50';
                    return `
                        <tr>
                            <td data-label="ID">${user.id}</td>
                            <td data-label="Profile Pic">
                                <img src="${profilePic}" alt="${user.name}" style="width: 50px; height: 50px; object-fit: cover; border-radius: 50%;">
                            </td>
                            <td data-label="Name">${user.name}</td>
                            <td data-label="Email">${user.email}</td>
                            <td data-label="Phone">${user.phone || 'N/A'}</td>
                            <td data-label="Orders">${userOrders.length}</td>
                            <td data-label="Joined On">${new Date(user.createdAt).toLocaleDateString()}</td>
                            <td data-label="Actions">
        ...
        const tableBody = document.getElementById('usersTableBody');
        if (tableBody) {
            tableBody.innerHTML = '<tr><td colspan="8" style="text-align: center;">Error loading user data. Is the server running?</td></tr>';
        }
    }
}
...
function addCategory(event) {
    ...
    const existingImageUrl = document.getElementById('categoryImage').value;
    ...
    if (imageFile) {
        ...
        reader.onload = (e) => createCategory(e.target.result);
        reader.readAsDataURL(imageFile);
    } else {
        createCategory(existingImageUrl || 'https://via.placeholder.com/150');
    }
}
...
function populateCategoryDropdown() {
    ...
    if (categorySelect) {
        const currentCategory = categorySelect.value; // Save current value if editing
        ...
        if (currentCategory) categorySelect.value = currentCategory; // Restore if editing
    }
}
...
function editProduct(productId) {
    ...
    if (product) {
        ...
        showAddProductModal();
        populateCategoryDropdown(); // Populate dropdown after showing modal
    }
}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Add event listener for category image preview
